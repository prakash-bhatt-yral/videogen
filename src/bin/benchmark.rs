...
const INSTANCES: &[(&str, &str)] = &[
    ...
    // ),
    ("instance3", "https://videogen.prakash.yral.com"),
    // (
    //     "instance4",
...
fn build_workflow(prompt: &str, _frames: u32, is_t2v: bool) -> Value {
    ...
    w.insert(
        "267:225".into(),
        json!({"inputs": {"value": VIDEO_SECONDS}, "class_type": "PrimitiveInt"}),
    ); // seconds
    w.insert(
        "267:257".into(),
        json!({"inputs": {"value": 1080}, "class_type": "PrimitiveInt"}),
    ); // width
    w.insert(
        "267:258".into(),
        json!({"inputs": {"value": 1920}, "class_type": "PrimitiveInt"}),
    ); // height
    ...
    w.insert(
        "267:266".into(),
        json!({"inputs": {"value": prompt}, "class_type": "PrimitiveStringMultiline"}),
    );
    ...
    w.insert(
        "267:276".into(),
        json!({"inputs": {"image": "example.png"}, "class_type": "LoadImage"}),
    );
    ...
    w.insert(
        "267:217".into(),
        json!({"inputs": {"av_latent": ["267:215", 0]}, "class_type": "LTXVSeparateAVLatent"}),
    );
    ...
    w.insert(
        "267:246".into(),
        json!({"inputs": {"sampler_name": "euler_cfg_pp"}, "class_type": "KSamplerSelect"}),
    );
    w.insert(
        "267:211".into(),
        json!({"inputs": {"sigmas": "0.85, 0.7250, 0.4219, 0.0"}, "class_type": "ManualSigmas"}),
    );
    ...
    w.insert(
        "267:218".into(),
        json!({"inputs": {"av_latent": ["267:219", 0]}, "class_type": "LTXVSeparateAVLatent"}),
    );
...
async fn main() -> Result<()> {
    ...
    println!(
        "Submitting {total} jobs ({} prompts × {} instances)…\n",
        PROMPTS.len(),
        INSTANCES.len()
    );
    ...
    let mut csv =
        String::from("instance,prompt_idx,prompt_snippet,status,gen_time_s,output_file,error\n");
    for r in &results {
